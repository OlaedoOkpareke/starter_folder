...
```{r 1, fig.cap = "Table of the first few rows of the data"}
x = head(rent_clean) %>%
...
```{r 2, fig.cap = "Table with summaries of the Score and Security for all apartments"}
rent_clean %>% 
  summarise(mean=mean(rent_clean$Score),median=median(rent_clean$Score), mean1 = mean(rent_clean$Security),median2=median(rent_clean$Security)) %>%
  knitr::kable(col.names = c("Mean Score", "Median Score", "Mean Security","Median Security"), caption = "Table of Summaries for Apartment Maintenance Scores")
```
...
```{r 2,fig.cap = "Boxplot of median apartment scores by Apartment Type", echo = TRUE }
rent_clean %>%
